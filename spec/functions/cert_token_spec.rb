...
describe 'vault_get::cert_token' do
  ...
  it 'raises a Puppet error when auth fails' do
    ...
    expect {
      function.execute('https://vault.doesnotexist:8200')
  ...
  it 'logs on, requests a secret using a token, and returns the token wrapped in the Sensitive type' do
    connection = instance_double('Puppet::Network::HTTP::Connection', address: 'vault.doesnotexist')
    expect(Puppet::Network::HttpPool).to receive(:http_ssl_instance).with('vault.doesnotexist', 8200).and_return(connection)
    ...
    result = function.execute('https://vault.doesnotexist:8200')
    expect(result).to be_a(Puppet::Pops::Types::PSensitiveType::Sensitive)
    expect(result.unwrap).to eq('7dad29d2-40af-038f-cf9c-0aeb616f8d20')
  end

  it 'logs on, requests a secret using a token, and returns the token wrapped in the Sensitive type from VAULT_ADDR' do
